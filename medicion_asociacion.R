...
#' 
rubic_d_hel <- dist.ldc(mc_rubic, "hellinger", silent = T)
rubic_d_hel %>% tidy # Para evitar desbordar la consola
#' 
...
#' 
coldiss(rubic_d_hel, diag = T)
#' 
#' Mejorable el gráfico, quizá este es más explícito:
#' 
coldissgg(rubic_d_hel, ordered = T, nc = 4, fsz = 0)
#' 
#' Con valores de distancia sobreimpresos (se empastan un poco)
#' 
coldissgg(rubic_d_hel, ordered = T, nc = 4, fsz = 1.5)
#' 
...
png(
  ...
  width = 2400, height = 1200, pointsize = 35
)
coldiss(rubic_d_hel, diag = T,)
dev.off()
#' 
...
#' 
rubic_jac <- vegdist(mc_rubic, method = 'jac', binary = T)
rubic_jac %>% tidy # Mostrando sólo las primeras 10 combinaciones en modo data.frame
#' 
...
#' 
(1 - rubic_jac) %>% tidy %>% rename(similaridad=distance) #Similaridad
#'
...
#' 
betadiver(mc_rubic, method = 'j') %>% tidy
#' 
...
#' 
rubic_sor <- vegdist(mc_rubic, method = 'bray', binary = T)
rubic_sor %>% tidy
coldiss(rubic_sor, diag = T)
#' 
