...
table(grupos_complete_k2) #Importante, tener en cuenta los desiguales tamaños de los grupos
...
#' 
(m_amb_complete_k2 <- bci_env_grid %>%
    select_if(is.numeric) %>% select(-id) %>% 
    mutate(grupos_complete_k2) %>%
    st_drop_geometry() %>% 
    pivot_longer(-grupos_complete_k2, names_to = "variable", values_to = "valor"))
#' 
#' para ward_k2 por mi
#' 
...
m_amb_complete_k2 %>%
  group_by(variable) %>%
  summarise(
    p_valor_t = t.test(valor ~ grupos_complete_k2)$p.value,
    p_valor_w = wilcox.test(valor ~ grupos_complete_k2, exact = F)$p.value) %>%
  arrange(p_valor_t) %>%
  print(n=Inf)
#' 
#' para ward_k2 por mi
...
m_amb_complete_k2 %>% 
  group_by(variable) %>% 
  ggplot() + aes(x = grupos_complete_k2, y = valor, fill = grupos_complete_k2) + 
  geom_boxplot() + 
...
#' Mapas:
#' 
mapa_complete_k2 <- mapView(
  bci_env_grid %>% mutate(grupos_complete_k2),
  layer.name = 'Grupos (2) complete',
  alpha.regions = 0.6,
  map.types = 'OpenTopoMap',
  legend = T,
  col.regions = colores_grupos[1:2],
  zcol = 'grupos_complete_k2') %>%
  ...
  leaflet::setView(
    ...
    zoom = 15)
mapa_complete_k2
mapa_complete_k2 %>% mapshot(
  file = 'mapa_complete_k2.png', 
...
#' 
#' Mapa de una de las variables donde se presentó efecto de su promedio (p<0.01), en este caso, fósforo (`P`)
#' 
mapa_fosforo <- mapView(
  bci_env_grid,
  layer.name = 'Fósforo',
  alpha.regions = 0.6,
  map.types = 'OpenTopoMap',
  ...
  leaflet::setView(
    ...
    zoom = 15)
mapa_fosforo
mapa_fosforo %>% mapshot(
  file = 'mapa_fosforo.png', 
  remove_controls = c("zoomControl", "layersControl", "homeButton")
)
#' 
