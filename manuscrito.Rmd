...
Las carasteristicas de la varianza en los datos ambientales y de la comunidad de especies en BCI fueron estudiadas mediante análisis de sus componentes principales (PCA) y el índice de correlación biserial-puntual [@borcard_legendre]. Este método permitió resumir la multidimensionalidad de las variables, explicar la varianza y los posibles patrones que estos podrían seguir, lo que ayudó a determinar cuales variables ambientales inciden en la composición de especies de los cuadrantes.<!--\ref{fig:grafico_pca_suelo}se observan los cuadrantes agrupados por el método jerarquico UPGMA? y presentados sobre las escalas de varianza relativas del primer y segundo componentes principales.-->
...
![Correlacion entre la diversidad/abundancia de rubiaceas y las variables edafológicas \label{fig:panel_cor_suelo_abun_riq_rubic}](panel_cor_suelo_abun_riq_rubic_spear.png)
...
![Componentes príncipales de la varianza en las variables de suelo y geomorfología en BCI. \label{fig:pca_suelo_geomorf_br_stick}](env_suelo_geomorf_pca_br_stick.png)
...
Las especies con preferencia por el grupo 1 evaden sistemáticamente al grupo 2, esto sucede de la misma manera para las especies con preferencia por el grupo 2. Esta diferenciación en la ocurrencia de las especies no se explica del todo con las variables ambientales medidas en BCI y es posible que exista algun otro factor que determina la ordenación de esta comunidad de plantas.
La cobertura del dosel arbóreo podría ser una de estas variables a considerarse, puesto que estas especies dominantes en la comunidad son a la vez tolerantes a la sombra (@grandtner2013dictionary).  
