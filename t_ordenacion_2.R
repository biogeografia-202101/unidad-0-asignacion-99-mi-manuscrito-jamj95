...
(df_equivalencias <- data.frame(
  nombre_original = colnames(mc_rubic),
  colnames(mi_fam)))
bci_env_grid %>% tibble
#' 
...
#' 
#'#' Agregué mis "variables significativas" en rubiaceae
#' 
...
#' 
#'#' Quité Ca para ver que pasaba, igual no hay tanto solapamiento en rubic
#' 
env_selec2 <- bci_env_grid %>%
  select(
    ...
    geomorf_valle_pct,
    UTM.NS,
    Al, B, Cu, Fe, Mg, K, Mn, P, Zn, N, N.min., pH) %>% 
  st_drop_geometry
...
#' 
#'#' Ahora quito "UTM.NS" y potasio. El vif.cca sigue bajando de a poquito
#' 
...
#' 
#' #' Ahora quito magnesio, boro y valle, aunque no quería, es eso o elevacion media
#' 
